...
Webpage for the Finnish Xiangqi federation, based on [zolastrap](https://marcodpt.github.io/zolastrap/)

## How to build

You need to have [zola](https://www.getzola.org/) installed, then run

```
 zola serve
```

to live track and render your edits. Below some documentation from the original zolastrap template
...
Set this in `config.toml` under `[extra]`, for example:
`banner = "img/banner.jpeg"`. The path is resolved from `static/`.
